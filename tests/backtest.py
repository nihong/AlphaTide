...
for d in past_dates_str:
    ...
    os.system(f"python3 main.py --date {d}")
...
for t in trades:
    ...
    # 找到买入点 (买入日的次日开盘价买入)
    buy_idx = df[df['日期'] > pd.to_datetime(t['buy_date'])].index.min()
    if pd.isna(buy_idx): continue # 没机会买入
    ...
    for i in range(buy_idx, len(df)):
        ...
        signals, desc = risk.evaluate_a_share_exit_signals(t['symbol'], target_date=current_date)
        if signals:
            sell_idx = i
